...
from features.decomposition_features import extract_features as extract_tsdecomp_features
...
class AblationConfig:
    ...
    feature_source: str = "INTERNAL" # INTERNAL, STL, SSA
...
ABLATION_EXPERIMENTS = {
    ...
    # ========== Experiment 6: External Decomposition (tsdecomp) ==========
    'stl_multi': AblationConfig(
        name='STL-Multi',
        description='STL decomposition via tsdecomp (statsmodels)',
        feature_source='STL'
    ),
    'ssa_multi': AblationConfig(
        name='SSA-Multi',
        description='SSA decomposition via tsdecomp',
        feature_source='SSA'
    ),
}
...
class STDDecomposer:
    ...
    def extract_features_tsdecomp(self, window: np.ndarray) -> np.ndarray:
        """Extract features using tsdecomp package."""
        cfg = {
            "method": self.config.feature_source,
            "block_sizes": self.block_sizes,
            "params": {} # Add specific params if needed
        }
        
        feat_dict = extract_tsdecomp_features(window, cfg)
        
        # Flatten in a deterministic order
        # Order: RAW, then per-block T, S, D
        flat_features = []
        
        # RAW
        if "RAW" in feat_dict and self.config.use_raw:
            flat_features.append(feat_dict["RAW"])
            
        # Components
        for block in self.block_sizes:
            prefix = "SSA_" if self.config.feature_source == "SSA" else ""
            
            if self.config.use_trend:
                key = f"{prefix}T_L{block}"
                if key in feat_dict: flat_features.append(feat_dict[key])
                
            if self.config.use_seasonal:
                key = f"{prefix}S_L{block}"
                if key in feat_dict: flat_features.append(feat_dict[key])
                
            if self.config.use_dispersion:
                key = f"{prefix}D_L{block}"
                # For dispersion, we might want the scalar std or the full residual
                # The original code used np.std(residual) -> scalar
                # The adapter returns full residual vector
                # Let's compute scalar std from the residual vector to match original behavior
                if key in feat_dict:
                    resid = feat_dict[key]
                    disp_val = np.std(resid)
                    flat_features.append(np.array([disp_val]))

        # Stats (if not provided by tsdecomp, compute locally)
        if self.config.use_stats:
        ...
        return np.concatenate(flat_features).astype(np.float32)

    def extract_features(self, window: np.ndarray) -> np.ndarray:
        """Dispatch to appropriate extraction method."""
        if self.config.feature_source in ["STL", "SSA"]:
            return self.extract_features_tsdecomp(window)
